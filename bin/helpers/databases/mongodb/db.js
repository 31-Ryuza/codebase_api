...
const { MONGO } = require('../../../infra/configs/text');
...
class DB {
  ...
  async findManySorted(parameter, removeField, parameterSort) {
    const ctx = 'mongodb-findMany';
    const dbName = await this.getDatabase();
    const result = await mongoConnection.getConnection(this.config);
    if (result.err) {
    ...
    try {
      ...
      const connection = cacheConnection.db(dbName);
      const db = connection.collection(this.collectionName);
      const recordset = await db.find(parameter, removeField).sort(parameterSort).toArray();
      if (validate.isEmpty(recordset)) {
        return wrapper.error('Data Not Found , Please Try Another Input');
      }
      return wrapper.data(recordset);
  ...
  async updateOne(parameter, updateQuery) {
    ...
    try {
      ...
      const connection = cacheConnection.db(dbName);
      const db = connection.collection(this.collectionName);
      const data = await db.updateOne(parameter, updateQuery);
      if (data.result.nModified > 0) {
        const recordset = await this.findOne(parameter);
        return wrapper.data(recordset.data);
  ...
  async deleteOne(parameter) {
    ...
    const result = await mongoConnection.getConnection(this.config);
    if (result.err) {
      logger.log(ctx, result.err.message, MONGO.ERROR_CONNECTION);
      return result;
    }
    try {
      const cacheConnection = result.data.db;
      ...
      const { result: { n, ok } } = data;
      if (ok === 1) {
        if (n === 0) {
          return wrapper.error(MONGO.DATA_NOT_FOUND);
        }
        return wrapper.data(result);
      }
      return wrapper.error(MONGO.FAILED_DELETE);
    } catch (err) {
      logger.log(ctx, err.message, MONGO.ERROR_DELETE_MESSAGE);
      return wrapper.error(MONGO.ERROR_DELETE_DATA(err.message));
    }
  }
